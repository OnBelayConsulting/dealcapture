...
public class PowerProfileDayPrivateDetail {
    ...
    public PowerProfileDayPrivateDetail() {
        initializeGetterMap();
        initializeSetterMap();
    }
    ...
    public void copyFrom(PowerProfileDayPrivateDetail copy) {
        if (copy.dayOfWeek != null)
            this.dayOfWeek = copy.dayOfWeek;

        for (int i = 1; i < 25; i++) {
            this.setPowerFlowCode(i, copy.getPowerFlowCode(i));
        }
    }


    public void copyTo(PowerProfileDayDetail copy) {
        copy.setDayOfWeek(this.dayOfWeek);
        List<String> hours = new ArrayList<>(24);
        for (int i = 1; i < 25; i++) {
            hours.add(getPowerFlowCode(i).getCode());
        }
        copy.setHours(hours);
    }

    public void copyFrom(PowerProfileDayDetail copy) {
        if (copy.getDayOfWeek() != null)
            this.dayOfWeek = copy.getDayOfWeek();

        copyHoursIntoColumns(copy.getHours());
    }


    private void copyHoursIntoColumns( List<String> hours) {
        for (int i = 0; i <hours.size(); i++) {
            setPowerFlowCode(i + 1, PowerFlowCode.lookUp(hours.get(i)));
        }
    }
