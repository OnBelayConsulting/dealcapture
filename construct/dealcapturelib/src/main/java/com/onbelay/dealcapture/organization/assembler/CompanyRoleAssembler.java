...
public class CompanyRoleAssembler extends AbstractOrganizationRoleAssembler {

	@Override
	public OrganizationRoleSnapshot assemble(OrganizationRole role) {
		
		CompanyRole companyRole = (CompanyRole) role;
		CompanyRoleSnapshot companyRoleSnapshot = new CompanyRoleSnapshot();
		super.setCommonAttributes(role, companyRoleSnapshot);
		
		companyRole.getDetail().copyFrom(companyRole.getDetail());
	...
	public OrganizationRole assembleEntity(OrganizationRoleSnapshot snapshot) {
		...
		companyRole.createWith((CompanyRoleSnapshot)snapshot);
		return companyRole;
	}
