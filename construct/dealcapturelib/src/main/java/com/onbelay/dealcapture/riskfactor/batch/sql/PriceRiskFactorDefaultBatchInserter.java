...
@Transactional
public class PriceRiskFactorDefaultBatchInserter implements PriceRiskFactorBatchInserter {
	private static final Logger logger = LogManager.getLogger(PriceRiskFactorDefaultBatchInserter.class);

	@Value("${positionBatchSize:20}")
	protected int batchSize;


	public PriceRiskFactorDefaultBatchInserter() {
	}
	
	
	@Override
	public void saveRiskFactors(List<PriceRiskFactorSnapshot> riskFactors) {
		
		
		EntityManager entityManager = ApplicationContextFactory.getCurrentEntityManagerOnThread();


		PriceRiskFactorSqlMapper sqlMapper = new PriceRiskFactorSqlMapper(false);
